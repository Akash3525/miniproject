...
if(isset($_POST['sub']))
{
  ...
  $count=0;
if($dp=="Donor"){
    $q=$db->prepare("DELETE FROM `pdonor` WHERE id='$id'");
    $t=$db->prepare("DELETE FROM `plocation` WHERE id='$id'");

    $count=$db->query("SELECT * FROM pdonor WHERE id='$id'")->fetchColumn();
}
else {
    $q=$db->prepare("DELETE FROM `ppatient` WHERE id='$id'");
    $count=$db->query("SELECT * FROM ppatient WHERE id='$id'")->fetchColumn();
}
