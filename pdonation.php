...
<?php
$q=$db->query("SELECT * FROM ppatient");
while ($p=$q->fetch(PDO::FETCH_OBJ)) {
?>
<tr>
  <td><?= $p->id; ?></td>
...
if(isset($_POST['sub']))
{
  $did=$_POST['did'];
  $pid=$_POST['pid'];
  $count=$db->query("SELECT count(*) FROM pdonor WHERE id='$did'")->fetchColumn();


$q=$db->query("SELECT date FROM pdonor WHERE id='$did'");
$d=$q->fetchColumn();
$current=date("Y/m/d");
$month=((strtotime($current) - strtotime($d))/60/60/24)/30;
if($month<3.0)
...
if(!$count==0){
$qd=$db->query("SELECT * FROM pdonor WHERE id='$did'");
$pd=$qd->fetch(PDO::FETCH_OBJ);
$dname=$pd->name;
$bdgroup=$pd->bgroup;

$qp=$db->query("SELECT * FROM ppatient WHERE id='$pid'");
...
$date=$current;
 if($bdgroup==$bgroup){
  $t=$db->prepare("INSERT INTO pdonation(donationid,dname,pname,bgroup,date) VALUES (:donationid,:dname,:pname,:bgroup,:date)");
...
$u=$db->prepare("UPDATE `pdonor` SET `date`='$date' WHERE id='$did'");
