...
<?php
    if(isset($_POST['sub'])){
        ...
        $q=$db->prepare("INSERT INTO pdonor(id,name,bgroup,gender,age,weight,date,number,address) VALUES (:id,:name,:bgroup,:gender,:age,:weight,:date,:number,:address)");
        ...
        if($q->execute()){
			$city=$_POST['city'];
			$l=$db->prepare("INSERT INTO plocation(id,city) VALUES (:id,:city)");
			$l->bindValue('id',$id);
			$l->bindValue('city',$city);
		if($l->execute()){
